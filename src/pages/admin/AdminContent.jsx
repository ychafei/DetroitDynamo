  ...
  return (
    <div className="py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <h1 className="font-oswald text-3xl font-bold tracking-tight text-foreground mb-8">SITE CONTENT</h1>
        <div className="bg-card border border-border rounded-lg p-6 space-y-6">
          {DEFAULT_KEYS.map(({ key, label, type }) => (
            <div key={key}>
              <Label className="font-oswald tracking-wider uppercase text-xs text-muted-foreground">{label}</Label>
              {type === 'textarea' ? (
                <Textarea
                  value={items[key] || ''}
          ...
          <Button onClick={save} disabled={saving} className="bg-accent text-accent-foreground font-oswald tracking-wider uppercase hover:bg-accent/90">
